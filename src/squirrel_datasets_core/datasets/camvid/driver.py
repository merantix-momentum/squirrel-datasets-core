...
class CamvidDriver(RecordIteratorDriver, TwoDImageFileDriver):
    ...
    def load_sample(sample: Dict, parse_image: bool = True, parse_label: bool = True) -> Dict:
        ...
        """
        if parse_image:
            sample["image"] = CamvidDriver.load_image(sample["image_url"])
        if parse_label:
            sample["label"] = CamvidDriver.load_image(sample["label_url"])
        return sample
